...
def _measure(
    ...
    base_probs, base_value, _ = brain.forward(
        obs, h, action_mask=action_mask, temperature=1.0
    )
    ...
    for idx in range(obs.shape[0]):
        ...
        probs, value, _ = brain.forward(
            perturbed, h, action_mask=action_mask, temperature=1.0
        )
...
def main() -> int:
    ...
    _print_top(
        "Top features (RUNTIME: action mask + instincts)", runtime_rows, names, args.top
    )
