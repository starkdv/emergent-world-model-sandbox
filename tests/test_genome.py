...
class TestGenome:
    ...
    def test_random_genome_creation(self):
        ...
        assert genome.weights.dtype in (
            np.float32,
            np.float64,
        ), "Weights should be float type"
    ...
    def test_genome_from_weights(self):
        ...
        traits = {
            ...
            for name, (min_val, max_val) in trait_config.items()
        }
    ...
    def test_crossover_one_point(self):
        ...
        assert (
            len(unique_vals) <= 2
        ), f"Should have at most 2 unique values, got {len(unique_vals)}"
    ...
    def test_trait_inheritance(self):
        ...
        children = [
            Genome.mate(parent_a, parent_b, trait_mutation_std=0.01) for _ in range(10)
        ]
