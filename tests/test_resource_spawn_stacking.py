...
def test_resource_spawn_respects_no_stacking():
    ...
    # Note: randomness might cause spawn failure sometimes, but with 1.0 rate it really should spawn
    if after_spawn_count > before_spawn_count:
        print(f"\n✅ SUCCESS: New object spawned when space became available")
    else:
        print(
            f"\n⚠️  WARNING: No object spawned even though space was available (could be RNG)"
        )
...
def test_calamity_respawn_rate():
    ...
    # Trigger calamity
    print(f"\n✓ Running tick 100 (calamity strikes strike)...")
    world.update()
    ...
    print(
        f"  Destroyed: {destroyed} objects ({destroyed / count_before_calamity * 100:.1f}%)"
    )
    ...
    for tick in range(1, 11):
        ...
        print(
            f"  Tick {tick}: {current_count} objects (+{respawned} from post-calamity)"
        )

    final_count = len(world.objects)
    total_respawned = final_count - count_after_calamity
    # Handling div by zero if calamity failed (RNG)
