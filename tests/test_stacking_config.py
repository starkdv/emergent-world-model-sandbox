...
def test_strict_mode():
    ...
    world = World(width=10, height=10, seed=42, allow_stacking=False)  # STRICT MODE
    ...
    if result2:
        ...
        print(
            f"   Tile ({obj2.x},{obj2.y}) object count: {len(nearby_tile.object_ids)}"
        )
    ...
    if result3:
        ...
        print(
            f"   Tile ({obj3.x},{obj3.y}) object count: {len(nearby_tile.object_ids)}"
        )
...
def test_legacy_mode():
    ...
    world = World(width=10, height=10, seed=42, allow_stacking=True)  # LEGACY MODE
