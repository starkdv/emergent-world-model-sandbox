...
def test_energy_sustainability():
    ...
    for _ in range(initial_berries * 2):  # Try extra times for failed spawns
        ...
        if tile and tile.is_passable() and not tile.object_ids:
            ...
            berry.add_component(
                EdibleComponent(calories=config["resources"]["berry_calories"])
            )
    ...
    for i in range(5):
        x, y = random.randint(0, world_cfg["width"] - 1), random.randint(
            0, world_cfg["height"] - 1
        )
        tile = world.get_tile(x, y)
    ...
    initial_food = sum(
        1 for o in world.objects.values() if o.has_component(EdibleComponent)
    )
    ...
    for tick in range(100):
        ...
        if tick % 50 == 0:
            ...
            if alive:
                ...
                print(
                    f"Tick {tick:4d}: {len(alive)} alive, energy={min_energy:.0f}-{max_energy:.0f} (avg {avg_energy:.0f}), max_age={max_age}, food={food_count}"
                )
            else:
    ...
    if alive:
        ...
        for a in alive:
            print(
                f"  Agent {a.id}: age={a.age}, energy={a.energy:.0f}/{a.max_energy:.0f}, inventory={len(a.inventory)}"
            )
