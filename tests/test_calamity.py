...
def test_calamity():
    ...
    # Count object types
    plants = sum(
        1 for obj in world.objects.values() if obj.has_component(PlantComponent)
    )
    food = sum(
        1 for obj in world.objects.values() if obj.has_component(EdibleComponent)
    )
    ...
    print(
        f"  Plants: {plant_loss_pct:.1f}% (expected ~{calamity_cfg.get('destruction_rate') * 100:.0f}%)"
    )
    print(
        f"  Food: {food_loss_pct:.1f}% (expected ~{calamity_cfg.get('destruction_rate') * 100:.0f}%)"
    )
