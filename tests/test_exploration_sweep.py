...
def run_test(bonus_value, max_ticks=1000):
        ...
        for agent in alive_agents:
            if hasattr(agent, 'last_action_result'):
                action_log.append({
                    'tick': tick,
                    'agent_id': agent.id,
                    'action': str(agent.last_action_result.action) if hasattr(agent.last_action_result, 'action') else 'UNKNOWN',
    ...
    # Calculate metrics
    wait_pct = (len(df[df['action'].str.contains('WAIT', na=False)]) / total_actions * 100) if total_actions > 0 else 0
    eat_actions = df[df['action'].str.contains('EAT', na=False)]
    eat_success_rate = (eat_actions['success'].sum() / len(eat_actions) * 100) if len(eat_actions) > 0 else 0
    survival_time = tick
...
def main():
    ...
    for r in results:
        ...
        wait_score = 30 if 32 <= r['wait_pct'] <= 35 else max(0, 30 - abs(r['wait_pct'] - 33.5) * 2)
        eat_score = 30 if 4 <= r['eat_success_rate'] <= 6 else max(0, 30 - abs(r['eat_success_rate'] - 5) * 3)
        total_score = survival_score + wait_score + eat_score
        
        print(f"{r['bonus']:>8.2f} | {r['survival_time']:>9d} | {r['wait_pct']:>6.1f}% | {r['eat_success_rate']:>6.1f}% | {total_score:>5.1f}")
    
    # Find best
    best = max(results, key=lambda r: 
        min(r['survival_time'] / 1500, 1.0) * 40 +
        (30 if 32 <= r['wait_pct'] <= 35 else max(0, 30 - abs(r['wait_pct'] - 33.5) * 2)) +
        (30 if 4 <= r['eat_success_rate'] <= 6 else max(0, 30 - abs(r['eat_success_rate'] - 5) * 3))
    )
