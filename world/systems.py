...
class DecaySystem:
    ...
    def __init__(
        ...
        soil_system: "SoilDynamicsSystem" = None,
                ...
                for dx in range(-fert.radius, fert.radius + 1):
                    ...
                    if abs(dx) + abs(dy) <= fert.radius:
                        ...
                        fertilizers_to_apply.append(
                            (target_x, target_y, fert.fertility_boost)
                        )
        ...
        for obj_id, obj in world.objects.items():
            if obj.has_component(PlantComponent):
                ...
                if tile.is_plantable():
                    tile.fertility = max(
                        0.0, tile.fertility - self.fertility_consumption
                    )
...
class ResourceSpawnSystem:
    ...
    def update(self, world: "World") -> None:
        ...
        if (
            edible_count < self.min_resources
            and random.random() < self.safety_spawn_rate
        ):
    ...
    def _spawn_resource_near(
        ...
        for dx, dy in offsets:
            ...
            if resource_type == "berry":
                ...
                berry.add_component(
                    EdibleComponent(
                        calories=self.berry_calories, toxicity=0.0, freshness=1.0
                    )
                )
        ...
        for obj, te in effect_sources:
            ...
            if has_blocker:
                ...
                if current >= te.reclaim_interval:
                    ...
                    tiles_to_reclaim.append((obj.x, obj.y, te.reclaim_terrain, 0.3))
            ...
            for dy in range(-te.spread_radius, te.spread_radius + 1):
                for dx in range(-te.spread_radius, te.spread_radius + 1):
                    ...
                    for oid in tile.object_ids:
                        ...
                        if (
                            nearby_obj
                            and getattr(nearby_obj, "type_id", "") == te.spread_type_id
                        ):
                    ...
                    has_blocker = self._tile_has_blocker(
                        world, nx, ny, te.spread_blocked_by
                    )
                    if has_blocker:
