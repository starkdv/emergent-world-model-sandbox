...
def adaptive_mutation_std(
    ...
    if current_diversity < low_diversity_threshold:
        ...
        ratio = (current_diversity - low_diversity_threshold) / (
            high_diversity_threshold - low_diversity_threshold
        )
...
class EvolutionStats:
    ...
    def print_summary(self):
        ...
        if self.best_fitness_history:
            ...
            print(
                f"  Generation {self.generation-1}: {self.best_fitness_history[-1]:.1f}"
            )
            improvement = self.best_fitness_history[-1] - self.best_fitness_history[0]
            print(
                f"  Improvement: {improvement:+.1f} ({improvement/self.best_fitness_history[0]*100:+.1f}%)"
            )
        ...
        if self.avg_fitness_history:
            ...
            print(
                f"  Generation {self.generation-1}: {self.avg_fitness_history[-1]:.1f}"
            )
        ...
        if self.diversity_history:
            ...
            if self.diversity_history[-1] < 2.0:
                print(
                    f"  ⚠️  Low diversity - population converging (consider increasing mutation)"
                )
        ...
        if self.survival_rate_history:
            avg_survival = sum(self.survival_rate_history) / len(
                self.survival_rate_history
            )
