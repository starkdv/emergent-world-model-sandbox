...
def _execute_and_log(
    ...
    if agent.learning_enabled and agent.learner:
        reward = agent.learner.reward_shaper.calculate_reward(
            action, result, energy_before, agent.energy, agent, world,
        )
...
def _handle_death(
    ...
    terminal_obs = None
    if (Agent.world_model_logger is not None and agent.last_observation is not None) or \
       (agent.learning_enabled and agent.learner and agent.last_observation is not None):
        terminal_obs = agent.observe(world)

    if Agent.world_model_logger is not None and agent.last_observation is not None and terminal_obs is not None:
        Agent.world_model_logger.log_transition(
            tick=world.tick,
            agent=agent,
            action=Action.WAIT,
    ...
    if agent.learning_enabled and agent.learner and agent.last_observation is not None and agent.last_hidden_state is not None:
