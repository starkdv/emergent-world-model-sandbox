...
class AsyncWorldModelLogger:
    ...
    def __init__(
        ...
        # Observation width drives the obs_*/obs_next_* column count.
        # (Was hardcoded to 64 while observations are 72-dimensional,
        # which misaligned every CSV column after obs_63.)
        from utils.agents.perception import get_observation_size

        self.obs_dim = get_observation_size()
    ...
    def _writer_loop(self) -> None:
        ...
        while self._running or not self.log_queue.empty():
            ...
            should_flush = (
                len(self.transitions_buffer) >= self.batch_size
                or len(self.episodes_buffer) >= self.batch_size
                or len(self.world_states_buffer) >= self.batch_size
    ...
    def log_transition(
        ...
        row = [
            ...
            round(energy_before / agent.max_energy, 3),
            round(agent.energy / agent.max_energy, 3),
            agent.age,
            len(agent.inventory)
    ...
    def _queue_episode_end(
        ...
        avg_energy = (
            ep["energy_sum"] / ep["energy_count"] if ep["energy_count"] > 0 else 0
        )
