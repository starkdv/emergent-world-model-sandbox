...
class Camera:
    ...
    def world_to_screen(
        self, world_x: float, world_y: float, tile_size: int
    ) -> Tuple[int, int]:
        """
...
class PygameRenderer:
            ...
            if event.type == pygame.QUIT:
                ...
                # R - reset camera
                    ...
                    self.camera = Camera(
                        ...
                        - (self.window_width / self.tile_size) / 2,
                        y=self.world.height / 2
                        - (self.window_height / self.tile_size) / 2,
                        zoom=1.0,
                    )
    ...
    def render_world(self) -> None:
        ...
        end_x = min(
            self.world.width,
            int(self.camera.x + self.window_width / scaled_tile_size) + 2,
        )
        end_y = min(
            self.world.height,
            int(self.camera.y + self.window_height / scaled_tile_size) + 2,
        )
        ...
        for agent in self.world.agents.values():
            if (
                agent.alive
                and start_x <= agent.x < end_x
                and start_y <= agent.y < end_y
            ):
                pos = (agent.x, agent.y)
                ...
                if tile.object_ids:
                    ...
                    if render_obj is not None:
                        ...
                        if seed_comp is not None:
                            ...
                            seed_growth_ratio = min(
                                1.0,
                                seed_comp.time_in_soil / max(1, seed_comp.grow_time),
                            )
                            if agent_planted:
                        ...
                        if obj_color:
                            ...
                            if is_seed_in_soil:
                                ...
                                outline_color = (
                                    COLORS["seed_agent_planted"]
                                    if agent_planted
                                    else (255, 255, 255)
                                )
                                pygame.draw.polygon(
                                    self.screen, outline_color, diamond_pts, 1
                                )
                                ...
                                pygame.draw.circle(
                                    self.screen, obj_color, (center_x, center_y), radius
                                )
    ...
    def render_hud(self) -> None:
        ...
        # Header bar
        pygame.draw.rect(
            self.screen, COLORS["panel_header"], (padding, padding, panel_width, 28)
        )
        ...
        tick_text = self.font_small.render(
            f"Tick: {self.world.tick:,}", True, COLORS["text"]
        )
        ...
        obj_text = self.font_small.render(
            f"Objects: {len(self.world.objects)}", True, COLORS["text"]
        )
        ...
        agent_text = self.font_small.render(
            f"Agents: {len(self.world.agents)}", True, COLORS["text"]
        )
        ...
        if self.show_help:
            ...
            help_surface = pygame.Surface(
                (help_panel_width, help_panel_height), pygame.SRCALPHA
            )
            ...
            pygame.draw.rect(
                self.screen,
                COLORS["panel_border"],
                (help_x, help_y, help_panel_width, help_panel_height),
                1,
            )
    ...
    def render_tile_info(self) -> None:
        ...
        if agents_here:
            ...
            for idx, agent in enumerate(agents_here[:2]):  # Show up to 2 agents
                ...
                info_lines.append(
                    f"  Energy: {agent.energy:.1f}/{agent.max_energy:.1f} ({agent.energy/agent.max_energy*100:.0f}%)"
                )
                ...
                # Inventory
                info_lines.append(
                    f"  Inventory: {len(agent.inventory)}/{agent.inventory_size}"
                )
                ...
                # Traits
                info_lines.append(
                    f"  Metabolism: {agent.genome.traits['metabolism_rate']:.2f}"
                )
                info_lines.append(
                    f"  Vision: {agent.genome.traits['vision_radius']:.1f}"
                )
                info_lines.append(
                    f"  Speed: {agent.genome.traits['movement_speed']:.2f}"
                )
        ...
        if objects:
            ...
            for idx, obj in enumerate(objects[:3]):  # Show up to 3 objects
                ...
                if obj.has_component(PlantComponent):
                    ...
                    info_lines.append(
                        f"  Mature: {'Yes' if plant.is_mature() else 'No'}"
                    )
                    if plant.is_mature():
                ...
                if obj.has_component(SeedComponent):
                    ...
                    info_lines.append(
                        f"  Req. Fertility: {seed.required_fertility:.2f}"
                    )
                ...
                if obj.has_component(FertilizerComponent):
                    ...
                    info_lines.append(
                        f"  Duration: {fert.duration}/{fert.max_duration}"
                    )
